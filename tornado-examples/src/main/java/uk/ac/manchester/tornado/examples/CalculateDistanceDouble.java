...
public class CalculateDistanceDouble {
    ...
    public static void calculateDistance(double[] lat1, double[] lon1, double[] lat2, double[] lon2, double[] distance) {
        for (@Parallel int i = 0; i < lat1.length; i++) {
            double lat1Rad = TornadoMath.toRadians(lat1[i]);
            double lon1Rad = TornadoMath.toRadians(lon1[i]);
            double lat2Rad = TornadoMath.toRadians(lat2[i]);
            double lon2Rad = TornadoMath.toRadians(lon2[i]);

            double x = (lon2Rad - lon1Rad) * TornadoMath.cos((lat1Rad + lat2Rad) / 2);
    ...
    public static void main(String[] args) throws Throwable {
        ...
        for (Integer N : sizes) {
            ...
            double[] lat1 = new double[N];
            double[] lon1 = new double[N];
            double[] lat2 = new double[N];
            double[] lon2 = new double[N];
            double[] distance = new double[N];
            double[] distanceSeq = new double[N];

            Random random = new Random();

            for (int i = 0; i < N; i++) {
                // Random lat: -90 to +90
                lat1[i] = -90 + 180 * random.nextDouble();
                lat2[i] = -90 + 180 * random.nextDouble();

                // Random lon: -180 to +180
                lon1[i] = -180 + 360 * random.nextDouble();
                lon2[i] = -180 + 360 * random.nextDouble();
            }
            ...
            TaskGraph taskGraph = new TaskGraph("s0") //
                    ...
                    .task("t0", CalculateDistanceDouble::calculateDistance, lat1, lon1, lat2, lon2, distance) //
